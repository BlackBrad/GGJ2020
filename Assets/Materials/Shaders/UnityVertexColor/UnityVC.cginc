...
#if defined(INSTANCED_COLOR_TINTS)
	UNITY_INSTANCING_BUFFER_START(Props)
		UNITY_DEFINE_INSTANCED_PROP(fixed4, _InstancedColorTint)
	UNITY_INSTANCING_BUFFER_END(Props)
#endif

#if defined(BLEND_COLORS)
float4 _BlendColor;
sampler2D _BlendTexToColor2;
float4 _BlendTexToColor2_ST;
#endif


#if defined(_HOLES)
uniform int _HolePoints_Length = 0;
uniform float4 _HolePoints [20]; // (x, y, z) = position (w) = radius
#endif
...
float4 TexCoords_TP(VertexInput_VC v, float3 posWorld)
{
	float4 texcoord;
	texcoord.xy = float4(posWorld.xz,0,0) * _MainTex_ST.xy + _MainTex_ST.zw;
	texcoord.zw = float4(posWorld.xz,0,0) * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
	//texcoord.zw = TRANSFORM_TEX(((_UVSec == 0) ? v.uv0 : v.uv1), _DetailAlbedoMap);
	return texcoord;
}
...
#if UNITY_SPECCUBE_BOX_PROJECTION || UNITY_LIGHT_PROBE_PROXY_VOLUME || defined(_HOLES) || defined(VERTEX_SHADER_TRIPLANAR)
...
#if UNITY_SPECCUBE_BOX_PROJECTION || UNITY_LIGHT_PROBE_PROXY_VOLUME || defined(_HOLES)
	o.posWorld = posWorld.xyz;
#endif
	o.pos = UnityObjectToClipPos(v.vertex);

#ifdef VERTEX_SHADER_TRIPLANAR
	o.tex = TexCoords_TP(v, o.posWorld);
#else
...
#if defined(BLEND_COLORS)
	s.diffColor = lerp(s.diffColor, _BlendColor, tex2D(_BlendTexToColor2, i.tex));
#endif

#if defined(INSTANCED_COLOR_TINTS)
	UNITY_SETUP_INSTANCE_ID(i);
	s.diffColor *= UNITY_ACCESS_INSTANCED_PROP(Props, _InstancedColorTint);
#endif
...
#if defined(_HOLES)
	for (int idx = 0; idx < _HolePoints_Length; idx ++)
	{
		// Calculates the contribution of each point
		half di = distance(i.posWorld.xz, _HolePoints[idx].xz);
		clip(di - _HolePoints[idx].w);
	}
#endif
